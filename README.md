...
<p align="center">
  ...
  <p align="center">
    ...
    <img src="https://img.shields.io/badge/dbt--core-%3E%3D1.8.0-orange" alt="dbt-core"/>
...
fabric-spark-test:
  ...
  outputs:
    fabricspark-dev:
        ...
        api_calls_per_minute: 150                  # Process-wide Fabric REST call ceiling; 0 disables
        adaptive_polling: false                    # Opt in to Spark-telemetry-driven polling (HC only)
...
| `api_calls_per_minute`      | int    | `150`                                 | Process-wide ceiling on Fabric REST calls per minute, shared across all dbt threads for the same Fabric endpoint and identity. The default leaves headroom under Fabric's per-identity quota for other tooling and concurrent dbt processes. Set to `0` to disable the limiter; the shared `Retry-After` gate still applies. Ignored in local mode.                   |
| `adaptive_polling`          | bool   | `false`                               | Opt in to Spark-telemetry-driven statement poll scheduling in Fabric high-concurrency mode. Uses one extra monitor REPL per underlying Livy session; ignored in local mode. See [Adaptive polling](#adaptive-polling).                                                                                                                                                |
| **Other**                   |        |                                       |                                                                                                                                                                                                                                                                                                                                                                       |
...
is applied _after_ the session `conf`, so a key the profile defines (for example
...
Fabric REST quotas are shared per identity, not per dbt process. The adapter always uses
a process-wide throttle governor for Fabric REST calls (`api_calls_per_minute`, default
`150`) plus a shared `Retry-After` gate: if one thread receives a 429, all threads wait.
Setting `api_calls_per_minute: 0` disables only the rate limiter; the 429 gate remains.
Local mode is not governed.

Even with `adaptive_polling: false`, statement polling now backs off as a statement runs
longer instead of polling at a fixed interval. For long-running models this reduces call
volume while keeping completion detection close to the old behavior; measured reductions
versus the old fixed ramp are roughly 1.7x for a 60-second model, 5x for a 5-minute model
and 14x for a 40-minute model.

Set `adaptive_polling: true` to add Spark-telemetry-driven scheduling in Fabric
high-concurrency mode. The adapter acquires one extra monitor REPL per underlying Livy
session and reads Spark task counters for in-flight statements, so polling follows real
progress and the monitor cost is roughly constant as `threads` increases.

The monitor consumes one REPL slot. When `adaptive_polling` is enabled and
`spark.highConcurrency.max` is not set in `spark_config.conf`, the adapter sets it to
`max(5, min(threads + 2, 50))` to leave room for worker REPLs, dbt's main connection and
the monitor. If you set `spark.highConcurrency.max` yourself, that value is respected
verbatim; keep it at least `threads + 2` so the monitor lands on the same underlying Spark
session as the workers. If it cannot, or if telemetry fails later, the adapter logs a
warning and falls back to schedule-based polling. Telemetry is only a scheduling hint:
only the authoritative Livy statement GET can complete, fail or resolve a statement.
