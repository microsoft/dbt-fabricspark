...
def get_headers(credentials: SparkCredentials, tokenPrint: bool=False) -> dict[str, str]:
    ...
    headers = {
        ...
        'Authorization': f'Bearer {accessToken.token}'
    }
    if tokenPrint:
...
class LivyCursor:
    ...
    def _submitLivyCode(self, code):
        ...
        if self.livy_session.is_new_session_required == True:
            LivySessionManager.connect(self.credential)
            self.session_id = self.livy_session.session_id

        # Submit code
        data = {"code": code, "kind":"sql"}
        logger.debug(f"Submitted: {data} {self.connect_url + '/sessions/' + self.session_id + '/statements'}")
...
class LivySessionManager:
    ...
    @staticmethod
    def connect(credentials: SparkCredentials):
        ...
        if (__class__.livy_global_session == None):
            ...
            __class__.livy_global_session.create_session(data)
            __class__.livy_global_session.is_new_session_required = False
        elif  __class__.livy_global_session.is_new_session_required == True:
            __class__.livy_global_session.create_session(data)
            __class__.livy_global_session.is_new_session_required = False
        else:
