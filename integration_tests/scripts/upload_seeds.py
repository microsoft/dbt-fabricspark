#!/usr/bin/env python3
"""Upload / update dbt-adventureworks seed CSV files on GitHub Gist, then regenerate ci_seeds.yaml.

Only uploads CSV files — SQL models, YAML schemas, and macros are committed to git.
...
Usage:
    ...
    python upload_seeds.py /path/to/project   # explicit project dir
"""
...
def _find_csv_files(project_dir: Path) -> list[Path]:
    files = []
    for d in SCAN_DIRS:
        ...
        if target.is_dir():
            files.extend(
                ...
                if p.is_file() and p.suffix in EXTENSIONS and p.name != MANIFEST
            )
    return sorted(files)
...
def _write_manifest(project_dir: Path, gist_id: str, gist_owner: str, files: list[Path]) -> None:
    lines = [
        "# Manifest for downloading dbt-adventureworks seed CSV files from GitHub Gist.",
...
def main() -> int:
    ...
    files = _find_csv_files(project_dir)
    if not files:
        print(f"No CSV seed files found in {project_dir}", file=sys.stderr)
        return 1
    print(f"Found {len(files)} CSV seed files")
