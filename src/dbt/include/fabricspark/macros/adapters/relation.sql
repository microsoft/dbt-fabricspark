{% macro fabricspark__list_relations_without_caching(relation) %}
  {% call statement('list_relations_without_caching', fetch_result=True) -%}
    show table extended in {% if relation.database %}{{ relation.database }}.{% endif %}{{ relation.schema }} like '*'
  {% endcall %}
...
{% macro list_relations_show_tables_without_caching(schema_relation) %}
  ...
  {% call statement('list_relations_without_caching_show_tables', fetch_result=True) -%}
    show tables in {% if schema_relation.database %}{{ schema_relation.database }}.{% endif %}{{ schema_relation.schema }} like '*'
  {% endcall %}
