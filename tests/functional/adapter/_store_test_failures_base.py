...
class StoreTestFailuresBase:
    ...
    def run_tests_store_one_run_with_relations(self, project):
        """Single-run variant: verify every failure relation matches expected seed.

        Covers the same relation-equality assertions as the idempotent-pair
        variant but without the second ``test --store-failures`` invocation, so
        the sibling test file runs ~1 dbt-invocation faster and the two classes
        can run in parallel on separate xdist workers.
        """
        self.test_audit_schema = project.adapter.config.credentials.schema
    ...
    def run_tests_store_failures_and_assert(self, project):
        """Single-run variant: verify relations and result vector after one run.

        Idempotency of ``test --store-failures`` is covered by dbt-core's own
        test suite — here we only need to verify that a single run produces the
        expected failure rows and that the stored relations match the expected
        seeds.
        """
