...
class TestSnapshotCrossLakehouseWrite(unittest.TestCase):
    """Verify snapshot materialization passes model.database to get_or_create_relation.

    Bug: The snapshot materialization hardcoded ``database=none`` in the
    ``get_or_create_relation`` call, causing snapshots to always land in
    the profile's default lakehouse instead of honouring the model-level
    ``database`` config for cross-lakehouse writes.

    Fix: Changed ``database=none`` to ``database=model.database``.

    These tests read the snapshot macro source and verify the call site,
    consistent with the pattern used in TestEnsureDatabaseExists.
    """
    ...
    @classmethod
    def setUpClass(cls):
        with open(cls.MACRO_PATH) as f:
            cls.macro_src = f.read()

    def test_get_or_create_relation_uses_model_database(self):
        """get_or_create_relation must receive database=model.database, not none."""
        # Match the get_or_create_relation call and extract its database= argument.
        pattern = re.compile(
            r"get_or_create_relation\s*\(\s*database\s*=\s*(\S+)\s*,",
            re.MULTILINE,
        )
        match = pattern.search(self.macro_src)
        self.assertIsNotNone(match, "Could not find get_or_create_relation call in snapshot.sql")
        db_arg = match.group(1).rstrip(",")
        self.assertEqual(
            db_arg,
            "model.database",
            f"Expected database=model.database but found database={db_arg}. "
            "Snapshots must use model.database for cross-lakehouse writes.",
        )

    def test_get_or_create_relation_does_not_use_none(self):
        """Regression guard: database=none must not appear in get_or_create_relation."""
        pattern = re.compile(
            r"get_or_create_relation\s*\([^)]*database\s*=\s*none\b",
            re.MULTILINE | re.IGNORECASE,
        )
        match = pattern.search(self.macro_src)
        self.assertIsNone(
            match,
            "get_or_create_relation still uses database=none — snapshots will ignore "
            "the model's database config and land in the profile's default lakehouse.",
        )

    def test_ensure_database_exists_uses_model_database(self):
        """ensure_database_exists must also receive database=model.database."""
        pattern = re.compile(
            r"ensure_database_exists\s*\(\s*model\.schema\s*,\s*database\s*=\s*(\S+)\s*\)",
            re.MULTILINE,
        )
        match = pattern.search(self.macro_src)
        ...
        self.assertEqual(
            db_arg,
            "model.database",
            f"Expected ensure_database_exists(..., database=model.database) "
            f"but found database={db_arg}.",
        )
